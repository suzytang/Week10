...
public class DetailFragment extends Fragment {
    ...
    private class GetCoinTask extends AsyncTask<Void, Void, List<Coin>> {
        ...
        protected List<Coin> doInBackground(Void... voids) {
            coinDatabase = Room.databaseBuilder(getActivity(), CoinDatabase.class, "myDB").build();
            return coinDatabase.coinDao().searchCoins(getArguments().getString(ARG_ITEM_ID));
        }

        @Override
        protected void onPostExecute(List<Coin> coins) {
            for(Coin coin : coins) {
                if(coin.equals(getArguments().getString(ARG_ITEM_ID))) {
                    mCoin = coin;
                    updateUi();
                }
            }
